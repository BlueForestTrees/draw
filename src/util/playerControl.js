...
const playFilm = film => {
    ...
    film.f.player.startMoment = Date.now() - (film.f.ftz * film.f.config.imageDuration * (1/film.f.config.durationCoef));
};
const nextLoop = film => {
...
const nextImage = (film) => {
    const totalMs = film.f.imageCount * film.f.config.imageDuration * (1/film.f.config.durationCoef);
